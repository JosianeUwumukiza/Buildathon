...
if not EXA_API_KEY or not OPENAI_API_KEY or not SCRAPER_API_KEY:
    raise ValueError("Missing one or more required API keys. Please check your .env file.")
...
def search_exa(query):
    ...
    exa_payload = {"query": query, "numResults": 3}
    response = requests.post(exa_search_url, headers=exa_headers, json=exa_payload)
    response.raise_for_status()
    data = response.json()
    # Extract results from the top-level "results" key and get URLs from the "id" field
...
def get_openai_summary(content, url):
    # Build a prompt that includes a snippet (up to 4000 characters) of the page content and the URL
    prompt = (
...
def get_prices():
    ...
    # === Step 1: Search with Exa Search ===
    try:
        urls = search_exa(query)
    except Exception as e:
        return jsonify({"error": "Exa Search API request failed", "details": str(e)}), 500

    if not urls:
        return jsonify({"error": "No valid URLs found in Exa Search API response"}), 404

    # === Step 2: Crawl pages using ScraperAPI ===
    crawl_results = []
    for url in urls:
        try:
            ...
            # Skip URLs that fail to crawl
            print(f"Error crawling {url}: {e}")
            continue

    if not crawl_results:
        return jsonify({"error": "ScraperAPI failed to crawl all URLs"}), 500

    # === Step 3: Generate summaries with OpenAI ===
    summaries = []
